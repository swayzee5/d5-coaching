...
type Props = {
  sessions: Session[];
  programId: string;
  renameAction: (sessionId: string, programId: string, formData: FormData) => Promise<void>;
  duplicateAction: (sessionId: string, programId: string) => Promise<void>;
  deleteAction: (sessionId: string, programId: string) => Promise<void>;
};

export function TemplateSessionList({
  sessions,
  programId,
  renameAction,
  duplicateAction,
  ...
  const commitRename = (sessionId: string) => {
    ...
    startTransition(() => renameAction(sessionId, programId, fd));
    setEditingId(null);
  };
      ...
      {sessions.map((session, i) => (
        ...
        >
          {editingId === session.id ? (
            ...
            <Link
              href={`/programmes/${programId}/seances/${session.id}`}
          ...
          {editingId !== session.id && (
            <div className="flex items-center gap-1 pr-3 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <form action={duplicateAction.bind(null, session.id, programId)}>
                <button
                  type="submit"
                  className="p-1.5 text-gray-500 hover:text-brand-400 transition-colors rounded text-xs"
                >
              ...
              <ConfirmButton
                action={deleteAction.bind(null, session.id, programId)}
