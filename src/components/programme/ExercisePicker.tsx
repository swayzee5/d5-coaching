...
export function ExercisePicker({
  ...
  const allMuscles = useMemo(() => {
    ...
    library.forEach((ex) => ex.muscles.forEach((m) => set.add(m)));
    return Array.from(set).sort();
  }, [library]);

  const filtered = library.filter((ex) => {
    const matchSearch = ex.name.toLowerCase().includes(search.toLowerCase());
    const matchMuscle = !muscleFilter || ex.muscles.includes(muscleFilter);
              ...
              {library.length === 0 ? (
                <div className="py-12 text-center">
                  ...
                  <p className="text-gray-700 text-xs mt-1">
                    Ajoutez des exercices dans la section Bibliothèque
                  </p>
                </div>
              ) : filtered.length === 0 ? (
                  ...
                  {filtered.map((ex) => (
                    <div key={ex.id} className="flex items-center justify-between py-3 gap-3">
                      ...
                      <button
                        ...
                        className="shrink-0 px-3 py-1.5 bg-brand-500/10 hover:bg-brand-500 border border-brand-500/30 hover:border-brand-500 text-brand-400 hover:text-white rounded-lg text-xs font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                      >
