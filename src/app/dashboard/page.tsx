    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/activites" className="text-xs text-brand-400 hover:text-brand-300 font-medium">
            Voir tout →
          </Link>
        </div>
            ...
            {data.recentCompletions.map((completion) => {
              ...
              return (
                <Link
                  ...
                  className="flex items-center gap-3 bg-gray-800 hover:bg-gray-750 rounded-lg px-4 py-3 transition-colors"
                >
          ...
          <div className="ml-4 border-l border-gray-700 pl-4 flex gap-2 shrink-0">
            {(["DECLINED", "GHOST"] as ProspectStatus[]).map((s) => (
              ...
              >
                ...
                <span className="text-xl font-bold text-gray-500">{data.byStatus[s] ?? 0}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/groups/new" className="text-xs text-brand-400 hover:text-brand-300 font-medium">+ Nouveau</Link>
          </div>
            ...
            <div className="space-y-3">
              {data.activeGroups.map((group) => {
                const totalDays = group.participants.reduce(
                  (acc, p) => acc + challengeProgress({ day1Done: p.day1Done, day2Done: p.day2Done, day3Done: p.day3Done, day4Done: p.day4Done, day5Done: p.day5Done, day6Done: p.day6Done, day7Done: p.day7Done }),
                  0
                );
                const maxDays = group.participants.length * 7;
                const pct = maxDays > 0 ? Math.round((totalDays / maxDays) * 100) : 0;
                return (
                  <Link key={group.id} href={`/groups/${group.id}`} className="block bg-gray-800 hover:bg-gray-750 rounded-lg p-4 transition-colors">
          ...
          <Link href="/groups" className="block text-center text-xs text-gray-500 hover:text-gray-300 mt-4 pt-3 border-t border-gray-800">
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/prospects/new" className="text-xs text-brand-400 hover:text-brand-300 font-medium">+ Ajouter</Link>
          </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        ...
                        {p.summaries.length > 0 && <span className="ml-2 text-brand-400">❆ Résumé IA</span>}
                      </p>
                    </div>
                  </div>
          ...
          <Link href="/prospects" className="block text-center text-xs text-gray-500 hover:text-gray-300 mt-4 pt-3 border-t border-gray-800">
