...
async function getDashboardData() {
  ...
  let activeGroups: Awaited<ReturnType<typeof db.challengeGroup.findMany>> = [];
  let recentProspects: Awaited<ReturnType<typeof db.prospect.findMany>> = [];
  let activeClients = 0;
  ...
  try {
    recentCompletions = await db.sessionCompletion.findMany({
      ...
      include: {
        session: {
          include: {
            program: { include: { client: { select: { id: true, firstName: true, lastName: true } } } },
          },
        },
            ...
            {data.recentCompletions.map((c) => {
              ...
              return (
                <Link key={c.id} href={`/app-clients/${client.id}/programmes/${c.session.program.id}/seances/${c.session.id}/resultats`}
                  className="flex items-center gap-3 bg-gray-800 rounded-lg px-4 py-3 transition-colors hover:bg-gray-750">
                  ...
                  <p className="text-xs text-gray-400 shrink-0">{done}/{c.setResults.length}</p>
                </Link>
              );
            })}
          </div>
