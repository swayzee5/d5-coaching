...
async function getClientActivity() {
  let recentWorkouts: WorkoutRow[] = [];
  let unreadNotes: NoteRow[] = [];

  try {
    recentWorkouts = (await db.$queryRaw`
      SELECT
        ws.id,
        ws.completed_at,
        ws.duration_seconds,
  ...
  return { recentWorkouts, unreadNotes };
}
        ...
        <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-orange-300 uppercase tracking-wide">
                Notes clients non lues
              </h2>
              <span className="ml-1 px-2 py-0.5 bg-orange-500 text-white text-xs font-bold rounded-full">
                {data.unreadNotes.length}
              </span>
            </div>
            <form action={markAllNotesRead}>
              <button
                type="submit"
                className="text-xs text-orange-400 hover:text-orange-300 border border-orange-500/30 rounded-lg px-3 py-1.5 hover:bg-orange-500/10 transition-colors"
              >
                Tout marquer lu
              </button>
            </form>
          ...
          <div className="space-y-2">
            {data.unreadNotes.map((note) => (
              <div
                key={note.id}
                className="bg-gray-900 border border-orange-500/20 rounded-lg px-4 py-3"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-300 leading-relaxed">&ldquo;{note.content}&rdquo;</p>
                  </div>
                </div>
              </div>
            ))}
            ...
            {data.recentWorkouts.map((w) => (
              ...
              >
                <div className="w-8 h-8 rounded-full bg-brand-500/10 flex items-center justify-center shrink-0">
                  <span className="text-brand-400 font-bold text-xs">{w.first_name[0]}{w.last_name[0]}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-white font-medium">{w.first_name} {w.last_name}</p>
                  <p className="text-xs text-gray-500 truncate">{w.session_name}</p>
                </div>
                <div className="text-right shrink-0">
                  {w.duration_seconds && (
                    <p className="text-xs text-green-400 font-medium">{formatDuration(w.duration_seconds)}</p>
                  )}
                  <p className="text-xs text-gray-500">{timeAgo(w.completed_at)}</p>
                </div>
              </div>
            ))}
          </div>
