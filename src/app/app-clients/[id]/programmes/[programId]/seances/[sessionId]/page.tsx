...
export default async function SessionBuilderPage({ params }: { params: { id: string; programId: string; sessionId: string } }) {
  const { id: clientId, programId, sessionId } = params;
  const [session, library] = await Promise.all([
    db.trainingSession.findUnique({
      where: { id: sessionId },
      include: { exercises: { orderBy: { orderIndex: "asc" } }, program: { select: { id: true, name: true, clientId: true } } },
    }),
    db.exerciseLibrary.findMany({ where: { isActive: true }, orderBy: { name: "asc" }, select: { id: true, name: true, muscles: true, description: true } }),
  ]);
  ...
  return (
    <div className="p-6 max-w-3xl space-y-6">
      <div>
        <Link href={`/app-clients/${clientId}/programmes/${programId}`} className="text-gray-500 hover:text-gray-300 text-sm transition-colors">← {session.program.name}</Link>
        <div className="flex items-center justify-between mt-4">
          <div>
            <h1 className="text-xl font-bold text-white">{session.name}</h1>
            {session.dayOfWeek !== null && <p className="text-gray-400 text-sm mt-0.5">{DAY_NAMES[session.dayOfWeek]}</p>}
          </div>
          ...
          <>
            ...
            <div className="divide-y divide-gray-800/50">
              {session.exercises.map((ex) => (
                <ExerciseRow
                  key={ex.id}
                  exercise={{ id: ex.id, name: ex.name, sets: ex.sets, reps: ex.reps, restSeconds: ex.restSeconds, notes: ex.notes }}
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        ...
        <form action={addAction} className="flex gap-3">
          <input name="name" placeholder="Nom de l'exercice..." required className="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-brand-500 transition-colors" />
          ...
          <button type="submit" className="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg text-sm font-medium transition-colors">Ajouter</button>
        </form>
      </div>
    </div>
  );
