...
export default async function AppClientDetailPage({ params }: { params: { id: string } }) {
  ...
  const serializedFiles = client.nutritionFiles.map((f: any) => ({
    ...
    <div className="p-6 space-y-6 max-w-4xl">
      ...
      <div className="flex gap-3 flex-wrap p-4 bg-gray-900 border border-gray-800 rounded-xl">
        <form action={archiveAction}>
          <button type="submit" className={`px-5 py-2.5 rounded-lg text-sm font-semibold transition-colors ${client.isActive ? "bg-gray-700 hover:bg-gray-600 text-white" : "bg-green-600 hover:bg-green-500 text-white"}`}>
            {client.isActive ? "📂 Archiver" : "✅ Réactiver"}
          </button>
        </form>
        <form action={blockAction}>
          <button type="submit" className={`px-5 py-2.5 rounded-lg text-sm font-semibold transition-colors ${client.isBlocked ? "bg-green-600 hover:bg-green-500 text-white" : "bg-orange-600 hover:bg-orange-500 text-white"}`}>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-800/50">
                  {client.progressEntries.map((entry: any, i: number) => {
