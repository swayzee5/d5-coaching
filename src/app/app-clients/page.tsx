...
type ClientRow = {
  ...
  createdAt: Date;
  progressEntries: { id: string; weightKg: unknown; entryDate: Date }[];
  nutritionFiles: { id: string }[];
};
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/app-clients/nouveau" className="px-4 py-2 bg-brand-500 hover:bg-brand-600 text-white rounded-lg text-sm font-semibold transition-colors">
          + Nouveau client
        </Link>
      </div>
      ...
      {clients.length === 0 ? (
        <div className="text-center py-20 text-gray-500">
          ...
          <p className="text-sm mt-2 text-gray-600">Crée le premier compte pour qu'un client accède à l'app D5.</p>
          <Link href="/app-clients/nouveau" className="inline-block mt-5 px-5 py-2.5 bg-brand-500 hover:bg-brand-600 text-white rounded-lg text-sm font-semibold transition-colors">
            Créer un compte
          </Link>
        </div>
      ) : (
              ...
              <Link key={client.id} href={`/app-clients/${client.id}`}
                ...
                <div className="flex items-center gap-6">
                  <div className="text-right">
                    {latest && Number(latest.weightKg) > 0 ? (
                      <>
                        <p className="text-sm font-bold text-white">{Number(latest.weightKg).toFixed(1)} kg</p>
                        <p className="text-xs text-gray-600">{formatDate(latest.entryDate)}</p>
                      </>
                    ) : (
                  ...
                  <div className="flex flex-col gap-1 items-end">
                    <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold ${
                      hasNutrition ? "bg-green-500/10 text-green-400" : "bg-gray-800 text-gray-500"
                    }`}>{hasNutrition ? "Nutrition ✓" : "Sans nutrition"}</span>
                    {client.isRebootOnly && (
                      <span className="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-brand-500/10 text-brand-400">Reboot only</span>
                    )}
                  </div>
                </div>
              </Link>
